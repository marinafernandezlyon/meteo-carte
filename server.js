...
app.get('/sort', function (req, res) {
  var indice;
  var cityListTmp = [];
  console.log(req.query.sortData);
  for(var i=0; i<req.query.sortData.length; i++) {
    indice = req.query.sortData[i];
    cityListTmp.push(cityList[indice]);
    console.log(cityList[indice]);
  }
  //console.log(cityListTmp);
...
app.get('/add', function (req, res) {
  ...
  request("http://api.openweathermap.org/data/2.5/weather?q="+req.query.city+"&APPID=9b754f1f40051783e4f72c176953866e&units=metric&lang=fr", function(error, response, body) {
    ...
    if(body.main != undefined) {
      ...
      req.query.lat       = body.coord.lat;
      req.query.lon       = body.coord.lon;
